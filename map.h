...
public:
    ...
    **/
    ...
    iterator find(const Key& key) {
    	...
    	while(indice->value()->key() != key){
    		if (indice->value()->key() < key)
    ...
    */
    const_iterator lower_bound(const Key& key) const {
        InnerNode* indice = header.parent;
        while (indice != nullptr && (indice->value()->key() < key || (indice->value()->key() != key && indice->child[0] != nullptr))) {
            if (indice->value()->key() < key)
                indice = indice->child[1];
            else
                indice = indice->child[0];
...
private:
     ...
     */
    struct InnerNode : public Node {
        InnerNode (value_type value) : color(Color::Red),  _value(value) {
            child[0] = child[1] = this;
            parent = nullptr;
        };
